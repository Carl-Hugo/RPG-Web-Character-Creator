        ...
        new Promise(resolve => {
            ...
            characters.forEach(character => {
                ...
                dataTypes.forEach((type, index) => {
                    db.doc(`users/${user}/data/characters/${character}/${type}/`)
                        .get()
                        .then(
                            doc => {
            ...
            Object.keys(customData).forEach((type, index) => {
                file[type] = {};
                db.doc(`users/${user}/customData/${type}/`)
                    .get()
                    .then(
                        doc => {
                            if (doc.exists) {
    ...
    render() {
        ...
        return (
            <Col sm="auto" className="align-self-end align-self-middle">
                                ...
                                <Row>
                                    <Label check>
                                        <Input
                                            ...
                                            checked={
                                                type === 'characters'
                                                    ? characters.length === Object.keys(list).length
                                                    : customDataTypes.every(
                                                          type =>
                                                              this.props[type]
                                                                  ? this.props[type]
                                                                      ? Object.keys(this.props[type]).every(key => (customData[type] ? customData[type].includes(key) : false))
                                                                      : false
                                                                  : false
                                                      )
                                            }
                                                    ...
                                                    <Input
                                                        ...
                                                        checked={
                                                            type === 'characters'
                                                                ...
                                                                : customData[item]
                                                                    ? Object.keys(this.props[item]).every(key => customData[item].includes(key))
                                                                    : false
                                                        }
                                                        value={item}
                                                        name={type}
                ...
                <Row>
                    <Button className="m-2 align-middle" onClick={this.generateExport}>
                        Export Selected{' '}
                    </Button>{' '}
