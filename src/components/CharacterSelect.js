...
class CharacterSelectComponent extends React.Component {
    ...
    render() {
        ...
        const { name, playerName, archetypeModal, careerModal, customCareersModal, customArchetypeModal, deleteModal, setting, discordPlayerId, discordChannelId } = this.state;
        return (
            <div className="w-100">
                ...
                <Row className="align-items-center">
                    ...
                    <Col md="auto">
                        <ButtonGroup>
                            ...
                            <Button onClick={() => this.setState({ deleteModal: true })} color="danger">
                                Delete
                            </Button>
                        </ButtonGroup>
                    </Col>
                ...
                <Row className="align-items-center">
                    ...
                    <Col className="m-auto">
                        <Input type="text" value={name ? name : ''} maxLength="50" name="name" onChange={this.handleChange} onBlur={() => this.props.changeCharacterName(name)} />
                    </Col>
                </Row>
                <hr />
                <Row className="align-items-center">
                    ...
                    <Col>{archetype === null ? '' : archetypes[archetype] ? archetypes[archetype].name : 'Missing Archetype Data'}</Col>
                    <Col>
                        <ButtonGroup>
                            <Button name="archetype" onClick={() => this.setState({ archetypeModal: true })}>
                                Select
                            </Button>
                            <Button
                ...
                <Row className="align-items-center">
                    ...
                    <Col>{career === null ? '' : careers[career] ? careers[career].name : 'Missing Career Data'}</Col>
                    <Col>
                        <ButtonGroup>
                            <Button name="career" onClick={() => this.setState({ careerModal: true })}>
                                Select
                            </Button>
                            <Button name="customCareer" onClick={() => this.setState({ customCareersModal: true })}>
                                Custom
                            </Button>
                        </ButtonGroup>
                    </Col>
                ...
                <CustomCareers modal={customCareersModal} handleClose={() => this.setState({ customCareersModal: false })} />
                <CustomArchetypes modal={customArchetypeModal} handleClose={() => this.setState({ customArchetypeModal: false })} />

                <ModalDeleteConfirm deleteModal={deleteModal} confirmedDelete={this.confirmedDelete} handleClose={() => this.setState({ deleteModal: false })} type="Character" />
            </div>
        );
    }
}
